...
static const BYTE privacy_sbox[16][16] = {
...
static const BYTE privacy_invsbox[16][16] = {
...
int encrypt_data(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[BLOCK_SIZE], buf_out[BLOCK_SIZE], iv_buf[BLOCK_SIZE];
	int blocks, idx;
	...
	for (idx = 0; idx < blocks; idx++) {
		memcpy(buf_in, &in[idx * BLOCK_SIZE], BLOCK_SIZE);
		xor_buf(iv_buf, buf_in, BLOCK_SIZE);
		encrypt(buf_in, buf_out, key, keysize);
		memcpy(&out[idx * BLOCK_SIZE], buf_out, BLOCK_SIZE);
		memcpy(iv_buf, buf_out, BLOCK_SIZE);
	}

	return(TRUE);
}

int decrypt_data(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[BLOCK_SIZE], buf_out[BLOCK_SIZE], iv_buf[BLOCK_SIZE];
	int blocks, idx;
	...
	for (idx = 0; idx < blocks; idx++) {
		memcpy(buf_in, &in[idx * BLOCK_SIZE], BLOCK_SIZE);
		decrypt(buf_in, buf_out, key, keysize);
		xor_buf(iv_buf, buf_out, BLOCK_SIZE);
		memcpy(&out[idx * BLOCK_SIZE], buf_out, BLOCK_SIZE);
		memcpy(iv_buf, buf_in, BLOCK_SIZE);
	}
...
WORD SubWord(WORD word)
{
	...
	result = (int)privacy_sbox[(word >> 4) & 0x0000000F][word & 0x0000000F];
	result += (int)privacy_sbox[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8;
	result += (int)privacy_sbox[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16;
	result += (int)privacy_sbox[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24;
	return(result);
}

void key_setup(const BYTE key[], WORD w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
...
{
	state[0][0] = privacy_sbox[state[0][0] >> 4][state[0][0] & 0x0F];
	state[0][1] = privacy_sbox[state[0][1] >> 4][state[0][1] & 0x0F];
	state[0][2] = privacy_sbox[state[0][2] >> 4][state[0][2] & 0x0F];
	state[0][3] = privacy_sbox[state[0][3] >> 4][state[0][3] & 0x0F];
	state[1][0] = privacy_sbox[state[1][0] >> 4][state[1][0] & 0x0F];
	state[1][1] = privacy_sbox[state[1][1] >> 4][state[1][1] & 0x0F];
	state[1][2] = privacy_sbox[state[1][2] >> 4][state[1][2] & 0x0F];
	state[1][3] = privacy_sbox[state[1][3] >> 4][state[1][3] & 0x0F];
	state[2][0] = privacy_sbox[state[2][0] >> 4][state[2][0] & 0x0F];
	state[2][1] = privacy_sbox[state[2][1] >> 4][state[2][1] & 0x0F];
	state[2][2] = privacy_sbox[state[2][2] >> 4][state[2][2] & 0x0F];
	state[2][3] = privacy_sbox[state[2][3] >> 4][state[2][3] & 0x0F];
	state[3][0] = privacy_sbox[state[3][0] >> 4][state[3][0] & 0x0F];
	state[3][1] = privacy_sbox[state[3][1] >> 4][state[3][1] & 0x0F];
	state[3][2] = privacy_sbox[state[3][2] >> 4][state[3][2] & 0x0F];
	state[3][3] = privacy_sbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

void InvSubBytes(BYTE state[][4])
{
	state[0][0] = privacy_invsbox[state[0][0] >> 4][state[0][0] & 0x0F];
	state[0][1] = privacy_invsbox[state[0][1] >> 4][state[0][1] & 0x0F];
	state[0][2] = privacy_invsbox[state[0][2] >> 4][state[0][2] & 0x0F];
	state[0][3] = privacy_invsbox[state[0][3] >> 4][state[0][3] & 0x0F];
	state[1][0] = privacy_invsbox[state[1][0] >> 4][state[1][0] & 0x0F];
	state[1][1] = privacy_invsbox[state[1][1] >> 4][state[1][1] & 0x0F];
	state[1][2] = privacy_invsbox[state[1][2] >> 4][state[1][2] & 0x0F];
	state[1][3] = privacy_invsbox[state[1][3] >> 4][state[1][3] & 0x0F];
	state[2][0] = privacy_invsbox[state[2][0] >> 4][state[2][0] & 0x0F];
	state[2][1] = privacy_invsbox[state[2][1] >> 4][state[2][1] & 0x0F];
	state[2][2] = privacy_invsbox[state[2][2] >> 4][state[2][2] & 0x0F];
	state[2][3] = privacy_invsbox[state[2][3] >> 4][state[2][3] & 0x0F];
	state[3][0] = privacy_invsbox[state[3][0] >> 4][state[3][0] & 0x0F];
	state[3][1] = privacy_invsbox[state[3][1] >> 4][state[3][1] & 0x0F];
	state[3][2] = privacy_invsbox[state[3][2] >> 4][state[3][2] & 0x0F];
	state[3][3] = privacy_invsbox[state[3][3] >> 4][state[3][3] & 0x0F];
}
...
void encrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
	state[0][0] = in[0];
	state[1][0] = in[1];
...
void decrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
	BYTE state[4][4];
	state[0][0] = in[0];
	state[1][0] = in[1];
